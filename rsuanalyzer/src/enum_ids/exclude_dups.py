...
from rsuanalyzer.src.calc_rsu._conf_id_to_lig_con_types import (
...
def _id_to_dup_ids(conf_id: str, theta: float) -> set[str]:
    ...
    # Type 1: Different Cut Points.
    orig_and_dups.update(_different_cut_points(conf_id))
...
def exclude_duplicates(
        conf_ids_with_dups: Iterable[str], theta: float) -> set[str]:
    ...
    while conf_ids_with_dups:
        ...
        dups_of_cur_id = _id_to_dup_ids(cur_id, theta)
